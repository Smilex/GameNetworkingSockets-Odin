...
remote_port : i32 = 43577
local_port : i32 = 43578
...
main :: proc() {
    ...
    listen_sock = gns.CreateListenSocketP2P(gns_interface, local_port, 0, nil );
    ...
    for {
        ...
        if connection != 0 {
            messages: [1]^gns.Message
            num_msgs := gns.ReceiveMessagesOnConnection(gns_interface, connection, raw_data(messages[:]), 1)
            if (num_msgs < 0) {
                return
            }
            if (num_msgs > 0) {
                msg := strings.string_from_ptr((^u8)(messages[0].m_pData), int(messages[0].m_cbSize))
                fmt.printf("%s\n", msg)
                gns.Release(messages[0])
            }
        }
